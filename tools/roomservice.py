...
try:
    device = proaospct[proaospct.index("_") + 1:]
except:
    device = proaospct

if not depsonly:
    print "Device %s not found. Attempting to retrieve device repository from aosp Github (http://github.com/Nougat-Athene)." % device

repositories = []

page = 1
while not depsonly:
    request = Request("https://api.github.com/users/Nougat-Athene/repos?page=%d" % page)
...
def get_from_manifest(devicename):
    try:
        lm = ElementTree.parse(".repo/local_manifests/aosp_manifest.xml")
        lm = lm.getroot()
    except:
...
def add_to_manifest_dependencies(repositories):
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            ...
            if existing_project.attrib['revision'] == repository['branch']:
                print 'Nougat-Athene/%s already exists' % (repo_name)
            else:
    ...
    f = open('.repo/local_manifests/aosp_manifest.xml', 'w')
    f.write(raw_xml)
    f.close()
    ...
    for repository in repositories:
        ...
        if existing_project != None:
            if existing_project.attrib['revision'] == repository['branch']:
                ...
                print 'updating branch for Nougat-Athene/%s to %s' % (repo_name, repository['branch'])
                existing_project.set('revision', repository['branch'])
            continue

        print 'Adding dependency: Nougat-Athene/%s -> %s' % (repo_name, repo_target)
        project = ElementTree.Element("project", attrib = { "path": repo_target,
            "remote": "github", "name": "Nougat-Athene/%s" % repo_name, "revision": "m" })
...
print "Repository for %s not found in the aosp Github repository list. If this is in error, you may need to manually add it to .repo/local_manifests/aosp_manifest.xml" % device
